...
import AttachmentPicker, { SelectedFile, StagedFilePreview } from "./Attachmentpicker";
...
export default function MessageInput({
  ...
  useEffect(() => {
    if (!isMobile) { setMobileInputVisible(true); return; }
  ...
  const otherMembers = (groupMembers ?? []).filter((m) => m._id !== currentUserId);
  ...
  const pickMention = (member: (typeof otherMembers)[0]) => {
    ...
    setSelectedMentionIds((prev) => prev.includes(member._id) ? prev : [...prev, member._id]);
  ...
  useEffect(() => { onMentionsChange?.(selectedMentionIds); }, [selectedMentionIds]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => { if (message === "") setSelectedMentionIds([]); }, [message]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (mentionQuery !== null && filteredMembers.length > 0) {
      if (e.key === "ArrowDown") { e.preventDefault(); setActiveMentionIndex((i) => (i + 1) % filteredMembers.length); return; }
      if (e.key === "ArrowUp") { e.preventDefault(); setActiveMentionIndex((i) => (i - 1 + filteredMembers.length) % filteredMembers.length); return; }
      if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); pickMention(filteredMembers[activeMentionIndex]); return; }
      if (e.key === "Escape") { setMentionQuery(null); return; }
    }
    if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); flushAndCall(handleSend); }
  };
  ...
  return (
    <div className={`flex-col mb-3 mx-3 overflow-hidden shadow flex border-1 min-w-0 border-base-content/10 rounded-2xl transition-all
        ...
        {isBlockedByMe ? (
          ...
          <>
            ...
            <AnimatePresence initial={false}>
              {replyingTo && (
                <motion.div key="reply" layout initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                  <div className="text-sm border-b border-base-content/20 bg-base-200 px-4 py-2 flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-base-content/70 text-xs leading-snug line-clamp-1">
                        Replying to <strong>{replyingTo.sender.displayName || replyingTo.sender.username}</strong>: {replyingTo.content}
                      </p>
                    </div>
                    <button aria-label="close" onClick={() => setReplyingTo(null)} className="opacity-60 hover:opacity-100 cursor-pointer p-1 rounded-full text-base-content flex-shrink-0">
                      <X size={18} />
                    </button>
                  </div>
                </motion.div>
            ...
            <AnimatePresence initial={false}>
              {editingMessage && (
                <motion.div key="edit" layout initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                  <div className="text-sm border-b border-base-content/20 bg-base-200 pl-7 px-4 text-base-content/70 py-2 flex justify-between w-full items-center">
                    ...
                    <button aria-label="close" onClick={() => setEditingMessage(null)} className="opacity-60 hover:opacity-100 cursor-pointer p-1 rounded-full text-base-content">
                      <X size={18} />
                    </button>
                  </div>
                </motion.div>
            ...
            <AnimatePresence initial={false}>
              {isGroup && mentionQuery !== null && (
                <motion.div key="mention-picker" layout initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                  <div className="border-b border-base-content/15 bg-base-200">
                    <div className="flex items-center gap-1.5 px-4 pt-2 pb-1">
                      <AtSign size={13} className="text-base-content/40" />
                      <span className="text-[11px] font-semibold uppercase tracking-wider text-base-content/40">Mention a member</span>
                    </div>
                    <div ref={mentionListRef} className="overflow-y-auto px-2 pb-2" style={{ maxHeight: "11.5rem" }}>
                      {filteredMembers.length === 0 ? (
                        <p className="text-xs text-base-content/40 px-2 py-3 text-center">No members match</p>
                      ) : (
                        filteredMembers.map((member, idx) => (
                          <button
                            key={member._id} type="button" data-mention-index={idx}
                          ...
                          >
                            <ProfilePicture src={member.profilePicture?.url ?? "/default-pfp.png"} size="sm" showStatus={false} />
                            <div className="min-w-0">
                              <p className="text-sm font-semibold text-base-content truncate leading-tight">{member.displayName || member.username}</p>
                              {member.displayName && <p className="text-[11px] text-base-content/50 truncate">@{member.username}</p>}
                            </div>
                            {idx === activeMentionIndex && <span className="ml-auto text-[10px] text-base-content/30">↵</span>}
                          </button>
                        ))
                      )}
                    </div>
            ...
            {/* Staged file preview */}
            {stagedFile && (
              <StagedFilePreview stagedFile={stagedFile} onRemove={removeStagedFile} />
            )}
            ...
            <div className="flex-none bg-base-100 w-full p-3 shadow flex items-end gap-2">
              ...
              {isMobile ? (
                mobileInputVisible ? (
                  ...
                  <div className="flex-1 min-w-0 text-base-content/40 px-4 py-2 rounded-md text-sm leading-relaxed" style={{ minHeight: "36px" }}>
                    Write a message...
                  </div>
                )
              ) : (
              ...
              <EmojiPickerBox
                ...
                onEmojiClick={(emoji: string) => {
                  if (isMobile && textareaRef.current) {
                    ...
                    setEditingMessage((prev) => prev ? { ...prev, content: prev.content + emoji } : null);
                  } else {
