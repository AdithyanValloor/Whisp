    ...
    <div className="flex-col mb-3 mx-3 overflow-hidden shadow flex border border-base-content/5 rounded-2xl">
      {replyingTo && (
        <div className="text-sm border-b border-base-content/20 bg-base-200 pl-7 px-4 text-base-content/70 py-2 flex justify-between w-full items-center">
          <span>
            Replying to{" "}
            <strong>{replyingTo.sender.displayName || replyingTo.sender.username}</strong>:{" "}
            {replyingTo.content}
          </span>
          <button 
            aria-label="close" 
      ...
      <div className="flex-none bg-base-100 w-full p-3 shadow flex items-end gap-2">
        ...
        <TextareaAutosize
          ...
          className="flex-1 resize-none px-4 py-2 focus:outline-none bg-transparent rounded-md text-sm leading-relaxed scrollbar-thin scrollbar-thumb-base-300 scrollbar-track-transparent max-h-40 overflow-y-auto"
          />
